...
namespace MechJebLibTest.PSGTests.AscentTests
{
    public class RealRocketTests
    {
        ...
        public void Delta3GTO()
        {
            ...
            // soft optimizer output: the total time-to-go converges to slightly different values across
            // runtimes/platforms (the tight orbital-target asserts below still pin the actual solution).
            solution.Tgo(0).ShouldEqual(924.139, 5e-2);
        ...
        public void Falcon9ExpendableGTO()
        {
            ...
            Rad2Deg(heading).ShouldEqual(89.53966801098629, 1e-1);

            (V3 rf1, _) = solution.StateVectors(solution.EndTime(1));

            (rf1.magnitude - R_BODY).ShouldEqual(101459.38840901759, 2.5e-1);

            (V3 rf, V3 vf) = solution.TerminalStateVectors();

            (double smaf, double eccf, double incf, double lanf, double argpf, _, _) =
                Astro.KeplerianFromStateVectors(MU, rf, vf);
            ...
            // terminal true anomaly is the free endpoint and wraps across 2*pi between runtimes
