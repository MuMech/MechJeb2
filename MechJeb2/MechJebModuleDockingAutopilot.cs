...
namespace MuMech
{
    public class MechJebModuleDockingAutopilot : ComputerModule
    {
        ...
        public override void Drive(FlightCtrlState s)
        {
            ...
            var targetVel = core.target.TargetOrbit.GetVel();

            var zApproachSpeed = MaxSpeedForDistance(Math.Max(zSep - acquireRange, 0), -zAxis);
            var latApproachSpeed = MaxSpeedForDistance(lateralSep.magnitude, -lateralSep); // TODO check if it should be +lateralSep
            ...
            var adjustment = (-lateralSep.normalized * latApproachSpeed) + (zApproachSpeed * zAxis);
        ...
        void UpdateDistance()
        {
            var separation = core.target.RelativePosition;
