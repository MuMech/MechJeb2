namespace MuMech.AttitudeControllers
{
    public class PIDLoop
    {
        ...
        public double Update(double reference, double measured)
        {
            ...
            var ep = (B * reference) - measured;
            var ei = reference - measured;
            var ed = (C * reference) - measured;

            // trapezoidal PID with derivative filtering as a digital biquad filter
            var a0 = (2 * N * Ts) + 4;
            var a1 = -8 / a0;
            var a2 = ((-2 * N * Ts) + 4) / a0;
            var b0 = ((4 * Kp*ep) + (4 * Kd*ed * N) + (2 * Ki*ei * Ts) + (2 * Kp*ep * N * Ts) + (Ki *ei * N * Ts * Ts)) / a0;
            var b1 = ((2 * Ki*ei * N * Ts * Ts) - (8 * Kp*ep) - (8 * Kd*ed * N)) / a0;
            var b2 = ((4 * Kp*ep) + (4 * Kd*ed * N) - (2 * Ki*ei * Ts) - (2 * Kp*ep * N * Ts) + (Ki *ei * N * Ts * Ts)) / a0;
