...
namespace MechJebLibTest.PSGTests.AscentTests
{
    public class RealRocketTests
    {
        ...
        public void Falcon9GTO()
        {
            ...
            var          v0     = new V3(0.000000, 408.739353, 0.000000);
            const double T0     = 0;
            const double R_BODY = 6378145;
            const double SMA_T  = 24361140;
            const double ECC_T  = 0.7308;
            const double INC_T  = 28.5 * DEG2RAD;
            const double LAN_T  = 269.8 * DEG2RAD;
            const double ARGP_T = 130.5 * DEG2RAD;
            ...
            double peR = Astro.PeriapsisFromKeplerian(SMA_T, ECC_T);
            double apR = Astro.ApoapsisFromKeplerian(SMA_T, ECC_T);
            ...
            double firstMdot = FIRST_THRUST / (FIRST_ISP * G0);
            double firstBt   = (FIRST_STAGE_M0 - FIRST_STAGE_MF) / firstMdot;
            double secondMdot = SECOND_THRUST / (SECOND_ISP * G0);
            double secondBt   = (SECOND_STAGE_M0 - SECOND_STAGE_MF) / secondMdot;
            ...
            Ascent ascent = Ascent.Builder()
                ...
                .AddCoast(SECOND_STAGE_M0, 0, 1200, 1, 1, massContinuity: true)
                ...
                .SetTarget(peR, apR, peR, INC_T, LAN_T, ARGP_T, 0, false, true, true)
                .Build();
            ...
            solution.Tgo(solution.T0, 0).ShouldEqual(firstBt, 1e-3);
            solution.Tgo(solution.T0, 2).ShouldEqual(409.30220835026103, 1e-3);
            (solution.Tgo(solution.T0, 1) + solution.Tgo(solution.T0, 3)).ShouldBeLessThanOrEqual(secondBt);
            ...
            smaf.ShouldEqual(SMA_T, 1e-5);
            eccf.ShouldEqual(ECC_T, 1e-5);
            perf.ShouldEqual(peR, 1e-5);
            aprf.ShouldEqual(apR, 1e-5);
