...
namespace MuMech
{
    public class Solution
    {
        ...
        {
            var tbar = (t - t0) / t_scale;
            var tmin = segments[n].Tmin;
            var tmax = segments[n].Tmax;
        ...
        public void pitch_and_heading(double t, ref double pitch, ref double heading)
        {
            ...
            var headVec = pv - (Vector3d.Dot(pv, rbar) * rbar);
            var east = Vector3d.Cross(rbar, new Vector3d(0, 1, 0)).normalized;
            var north = Vector3d.Cross(east, rbar).normalized;
