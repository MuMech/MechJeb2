...
namespace MuMech
{
    public class OperationAdvancedTransfer : Operation
    {
        ...
        void ComputeTimes(Orbit o, Orbit destination, double universalTime)
        {
            ...
            var synodic_period = o.referenceBody.orbit.SynodicPeriod(destination);
            var hohmann_transfer_time = OrbitUtil.GetTransferTime(o.referenceBody.orbit, destination);
        ...
        private void DoPorkchopGui(Orbit o, double universalTime, MechJebModuleTargetController target)
        {
            ...
            var departure = " - ";
            var duration = " - ";
            if (worker.Finished && worker.Computed.GetLength(1) == porkchop_Height && plot == null && Event.current.type == EventType.Layout)
            {

                var width = worker.Computed.GetLength(0);
                var height = worker.Computed.GetLength(1);
                ...
                {
                    selectedPoint = new int[] { worker.BestDate, worker.BestDuration }
                };
            }
            if (plot != null)
            {
            ...
            {
                var progressStyle = new GUIStyle
                {
            ...
            {
                var bestDuration = 0;
                for (var i = 1; i < worker.Computed.GetLength(1); i++)
                {
        ...
        {
            // Check preconditions
            var message = CheckPreconditions(o, target);
            if (message != null)
            {
            ...
            var target_PeR = lastTargetCelestial.Radius + (periapsisHeight * 1000);
