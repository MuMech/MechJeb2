    ...
    namespace Landing
    {
        public class LowDeorbitBurn : AutopilotStep
        {
            ...
            public override AutopilotStep OnFixedUpdate()
            {
                ...
                var stoppingDistance = Math.Pow(vesselState.speedSurfaceHorizontal, 2) / (2 * vesselState.limitedMaxThrustAccel);
                var triggerDistance = lowDeorbitBurnTriggerFactor * stoppingDistance;
                var heightAboveTarget = vesselState.altitudeASL - core.landing.DecelerationEndAltitude();
                ...
                var rangeToTarget = Vector3d.Exclude(vesselState.up, core.target.GetPositionTargetPosition() - vesselState.CoM).magnitude;
                ...
                var thrustDirection = -vesselState.surfaceVelocity.normalized;
                ...
                if (deorbitBurnTriggered && core.landing.PredictionReady)
                {
                    ...
                    var horizontalToLandingSite = Vector3d.Exclude(vesselState.up, core.landing.LandingSite - vesselState.CoM).normalized;
                    var horizontalToTarget = Vector3d.Exclude(vesselState.up, core.target.GetPositionTargetPosition() - vesselState.CoM).normalized;
                    const double angleGain = 4;
                    var angleCorrection = angleGain * (horizontalToTarget - horizontalToLandingSite);
                    ...
                    var rangeToLandingSite = Vector3d.Exclude(vesselState.up, core.landing.LandingSite - vesselState.CoM).magnitude;
                    ...
                    if (orbit.PeA < 0)
                    {
                        if (rangeToLandingSite > rangeToTarget)
                        {
                            ...
                            var maxAllowedSpeedAfterDt = core.landing.MaxAllowedSpeedAfterDt(vesselState.deltaT);
                            var speedAfterDt = vesselState.speedSurface + (vesselState.deltaT * Vector3d.Dot(vesselState.gravityForce, vesselState.surfaceVelocity.normalized));
