...
namespace MuMech
{
    public class VesselState
    {
        ...
        // public Vector6 rcsTorqueAvailable = new Vector6(); // replaced by torqueRcs stock calculation
        ...
        void UpdateRCSThrustAndTorque(Vessel vessel)
        {
                    ...
                    if (throttles != null)
                    {
                        for (int j = 0; j < throttles.Length; j++)
                        {
                            if (throttles[j] > 0)
                            {
                                ...
                                // Are we missing a torqueRcs calculation here?
                            }
                        }
                    }
                }
            ...
            for (int i = 0; i < vessel.parts.Count; i++)
            {
                ...
                for (int m = 0; m < p.Modules.Count; m++)
                {
                    ...
                    /* this has been replaced by the torqueRcs stock calculation, kept here for a bit in case torqueRcs is still buggy
