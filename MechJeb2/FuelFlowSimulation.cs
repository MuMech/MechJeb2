...
namespace MuMech
{
    public partial class FuelFlowSimulation
    {
        ...
        public void Init(List<Part> parts, bool dVLinearThrust)
        {
            ...
            {
                var p = parts[i];
                var node = _nodeLookup[p];
                node.AddCrossfeedSouces(p.crossfeedPartSet.GetParts(), _nodeLookup);
                if (node.decoupledInStage >= _simStage)
                {
                    _simStage = node.decoupledInStage + 1;
                }
            }
        ...
        public FuelStats[] SimulateAllStages(float throttle, double staticPressureKpa, double atmDensity, double machNumber)
        {
            ...
            var staticPressure = staticPressureKpa * _kpaToAtmospheres;
        ...
        private FuelStats SimulateStage(float throttle, double staticPressure, double atmDensity, double machNumber)
        {
            ...
            for (var i = 0; i < _nodes.Count; i++)
            {
                _nodes[i].SetConsumptionRates(throttle, staticPressure, atmDensity, machNumber);
            }

            var fuelStats = new FuelStats
            {
                StartMass = VesselMass(_simStage),
                StartThrust = VesselThrust(),
                EndThrust = VesselThrust()
            };
            ...
            {
                fuelStats.Parts.Add(_partLookup[engines[i]]);
            }
            ...
            while (!AllowedToStage())
            {
                ...
                fuelStats = fuelStats.Append(SimulateTimeStep(double.MaxValue, throttle, staticPressure, atmDensity, machNumber, out var dt));
        ...
        private void SimulateStageActivation()
        {
            ...
            using var decoupledNodes = ListPool<FuelNode>.Instance.BorrowDisposable();

            _nodes.Slinq().Where((n, stage) => n.decoupledInStage >= stage, _simStage).AddTo(decoupledNodes);

            for (var i = 0; i < decoupledNodes.value.Count; i++)
            {
                var decoupledNode = decoupledNodes.value[i];
            ...
            for (var i = 0; i < _nodes.Count; i++)
            {
                for (var j = 0; j < decoupledNodes.value.Count; j++)
                {
        ...
        private bool AllowedToStage()
        {
            ...
            using (var activeEngines = FindActiveEngines())
            {
                ...
                using (var burnedResources = ListPool<int>.Instance.BorrowDisposable())
                {
                    ...
                    for (var i = 0; i < _nodes.Count; i++)
                    {
                        ...
                        if (n.decoupledInStage == _simStage - 1 && !n.isSepratron)
                        {
                            ...
                            {
                                var activeEnginesCount = activeEngines.value.Count;
                                for (var j = 0; j < activeEnginesCount; j++)
                                {
                                    var engine = activeEngines.value[j];
                ...
                {
                    var activeEnginesWorking = false;
                    var partDecoupledInNextStage = false;

                    for (var i = 0; i < _nodes.Count; i++)
                    {
                        var n = _nodes[i];
                        if (activeEngines.value.Contains(n) && n.CanDrawNeededResources(_nodes))
                        {
        ...
        private double VesselThrust()
        {
            ...
            for (var i = 0; i < activeEngines.value.Count; i++)
            {
