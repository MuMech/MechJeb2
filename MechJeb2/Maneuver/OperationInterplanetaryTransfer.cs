    ...
    public class OperationInterplanetaryTransfer : Operation
    {
        ...
        public override List<ManeuverParameters> MakeNodesImpl(Orbit o, double UT, MechJebModuleTargetController target)
        {
            ...
            if (o.referenceBody.referenceBody == null)
                throw new OperationException(Localizer.Format("#MechJeb_transfer_Exception2",o.referenceBody.displayName.LocalizeRemoveGender()));//doesn't make sense to plot an interplanetary transfer from an orbit around <<1>>

            if (o.referenceBody.referenceBody != target.TargetOrbit.referenceBody)
            {
                if (o.referenceBody == target.TargetOrbit.referenceBody)
                    throw new OperationException(Localizer.Format("#MechJeb_transfer_Exception3", o.referenceBody.displayName.LocalizeRemoveGender()));//use regular Hohmann transfer function to intercept another body orbiting <<1>>
            ...
            {
                errorMessage = Localizer.Format("#MechJeb_transfer_errormsg1", o.RelativeInclination(target.TargetOrbit).ToString("F0"), o.referenceBody.displayName.LocalizeRemoveGender());//"Warning: target's orbital plane is at a"<<1>>"º angle to "<<2>>"'s orbital plane (recommend at most 30º). Planned interplanetary transfer may not intercept target properly."
            }
            else
            {
