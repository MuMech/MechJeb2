            ...
            bool optguard = false)
        {
            ...
            if (optguard)
            {
                ...
                if (ogrep.badgradsuspected)
                    throw new Exception(
                        $"badgradsuspected: {ogrep.badgradfidx},{ogrep.badgradvidx}\nuser:\n{DoubleMatrixString(ogrep.badgraduser)}\nnumerical:\n{DoubleMatrixString(ogrep.badgradnum)}\nsparsity check:\n{DoubleMatrixSparsityCheck(ogrep.badgraduser, ogrep.badgradnum, 1e-4)}");

                if (ogrep.nonc0suspected)
                    throw new Exception($"nonc0suspected in fi[{ogrep.nonc0fidx}]");

                if (ogrep.nonc1suspected)
                    throw new Exception($"nonc1suspected in fi[{ogrep.nonc1fidx}]");
            }
