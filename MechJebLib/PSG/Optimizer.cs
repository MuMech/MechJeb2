...
namespace MechJebLib.PSG
{
    public class Optimizer : IDisposable
    {
        ...
        private int ContinuityConstraints(double[] f, alglib.sparsematrix j, int p, int ci, bool debug)
        {
            ...
            ci = ApplyVectorConstraintV3(f, j, ci, x => x[0] - x[1], new[] { prevPhase.R[-1], thisPhase.R[0] }, new[] { prevPhase.R.Idx(-1), thisPhase.R.Idx(0) });
            ci = ApplyVectorConstraintV3(f, j, ci, x => x[0] - x[1], new[] { prevPhase.V[-1], thisPhase.V[0] }, new[] { prevPhase.V.Idx(-1), thisPhase.V.Idx(0) });
            ci = ApplyVectorConstraintV3(f, j, ci, x => x[0] - x[1], new[] { prevPhase.U[-1], thisPhase.U[0] }, new[] { prevPhase.U.Idx(-1), thisPhase.U.Idx(0) });

            // mass continuity for coast-within-phase
            if (p > 0 && _phases[p].MassContinuity)
            {
                if (debug) DebugPrint($"mass continuity constraint: {ci - 1}");
                ci = ApplyScalarConstraint(f, j, ci, x => x[0] - x[1], new[] { prevPhase.M[-1], thisPhase.M[0] }, new[] { prevPhase.M.Idx(-1), thisPhase.M.Idx(0) });
            }
        ...
        private int StagingConstraints(double[] f, alglib.sparsematrix j, int p, int ci)
        {
            ...
            if (nextBurnPhaseIndex < 0 || _phases[p].Coast || !_phases[p].AllowShutdown || _phases[p].MassContinuity)
            {
                f[ci++] = 0;
            ...
            if (nextNextBurnPhaseIndex < 0 && _phases[nextBurnPhaseIndex].MassContinuity)
            {
                f[ci++] = 0;
                alglib.sparseappendemptyrow(j);
                return ci;
            ...
            bool combiningThisBurn = nextNextBurnPhaseIndex > 0 && _phases[nextBurnPhaseIndex].MassContinuity;
            bool combiningNextBurn = nextNextBurnPhaseIndex > 0 && !combiningThisBurn && _phases[nextNextBurnPhaseIndex].MassContinuity;
        ...
        private int DynamicConstraints(double[] f, alglib.sparsematrix j, int p, int ci, bool debug)
        {
            ...
            for (int n = 0; n < N - 1; n += 1)
            {
                ...
                if (!_phases[p].Coast)
                {
                    ...
                    bool doingMassContinuity = p > 0 && _phases[p].MassContinuity;

                    double mi = doingMassContinuity ? thisPhase.M[0] : _phases[p].m0;
