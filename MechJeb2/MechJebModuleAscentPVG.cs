...
namespace MuMech
{
    public class MechJebModuleAscentPVG : MechJebModuleAscentBase
    {
        ...
        {
            var PeR = mainBody.Radius + PeA;
            var ApR = mainBody.Radius + ApA;
        ...
        private void ConvertToVTRT(double sma, double ecc, double attachAlt, out double gammaT, out double rT, out double vT)
        {
            ...
            var h = Math.Sqrt(mainBody.gravParameter * sma * (1 - (ecc * ecc)));
        ...
        private void SetTarget()
        {
            ...
            ConvertToSMAEcc(autopilot.desiredOrbitAltitude, DesiredApoapsis, out var sma, out var ecc);
            ConvertToVTRT(sma, ecc, AttachAltFlag ? attachAlt : autopilot.desiredOrbitAltitude, out var gammaT, out var rT, out var vT);
            var inclination = autopilot.desiredInclination;
        ...
        private void DriveVerticalAscent(FlightCtrlState s)
        {
            ...
            else
            {
                ...
                var dv = PitchStartVelocity - vesselState.surfaceVelocity.magnitude;
        ...
        {
            var dt = autopilot.MET - _pitchStartTime;
            var theta = dt * PitchRate;
            var pitch = 90 - theta;
        ...
        private bool CheckForGuidanceTransition(double pitch)
        {
            ...
            if (!StagingTriggerFlag)
            {
                ...
                if (vesselState.maxDynamicPressure > vesselState.dynamicPressure * 1.1 && vesselState.dynamicPressure < DynamicPressureTrigger)
                {
                    return true;
                }
            }
            ...
            {
                if (core.guidance.isStable() && core.guidance.solution.arc(0).rocket_stage >= StagingTrigger)
                {
                    return true;
                }
            }
        ...
        private void DriveGuidance(FlightCtrlState s)
        {
            ...
            {
                var pitch = Math.Min(Math.Min(90, srfvelPitch()), vesselState.vesselPitch);
