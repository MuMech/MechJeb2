...
namespace MechJebLib.PSG
{
    public partial class Ascent
    {
        public class AscentBuilder
        {
            ...
            {
                DebugPrint(
                    $"[MechJebLib.AscentBuilder] AddStageUsingFinalMass({m0}, {mf}, {isp}, {bt}, {kspStage}, {(unguided ? "true" : "false")}, {(allowShutdown ? "true" : "false")})");
            ...
            {
                DebugPrint(
                    $"[MechJebLib.AscentBuilder] AddStageUsingThrust({m0}, {thrust}, {isp}, {bt}, {kspStage}, {(unguided ? "true" : "false")}, {(allowShutdown ? "true" : "false")})");
            ...
            public AscentBuilder AddFixedCoast(double m0, double ct, int kspStage, int mjPhase, bool unguided = false)
            {
                DebugPrint($"[MechJebLib.AscentBuilder] AddFixedCoast({m0}, {ct}, {kspStage})");

                _phases.Add(Phase.NewFixedCoast(m0, ct, kspStage, mjPhase, unguided));

                return this;
            }

            public AscentBuilder AddOptimizedCoast(double m0, double mint, double maxt, int kspStage, int mjPhase, bool unguided = false)
            {
                DebugPrint($"[MechJebLib.AscentBuilder] AddOptimizedCoast({m0}, {mint}, {maxt}, {kspStage})");

                _phases.Add(Phase.NewOptimizedCoast(m0, mint, maxt, kspStage, mjPhase, unguided));
