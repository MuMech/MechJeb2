    ...
    public class RCSSolver
    {
        public class Thruster
        {
            ...
            public Vector3 GetThrust(Vector3 direction, Vector3 rotation)
            {
                ...
                {
                    var torque = -Vector3.Cross(pos, thrustDir);
                    var translateThrottle = Vector3.Dot(direction, thrustDir);
                    var rotateThrottle = Vector3.Dot(rotation, torque);
                    var throttle = Mathf.Clamp01(translateThrottle + rotateThrottle);
        ...
        protected void cost_func(double[] x, ref double func, object obj) {
            func = 0;
            for (var attr = 0; attr < B.Length; attr++) {
                double tmp = 0;
        ...
        public double[] run(List<Thruster> fullThrusters, Vector3 direction, Vector3 rotation)
        {
            ...
            var fullCount = fullThrusters.Count;
            ...
            var bndl = new double[count];
            var bndu = new double[count];

            // Initialize the matrix based on thruster directions.
            var tIdx = -1;
            for (var i = 0; i < fullCount; i++)
            {
                var thrust = thrustForces[i];
                if (thrust.magnitude == 0)
                {
                    continue;
                }
                var thruster = thrusters[++tIdx];

                var torque = thruster.GetTorque(thrust);
                var thrustNorm = thrust.normalized;

                var torqueErr = torque - rotation;
                var transErr = thrustNorm - direction;
                ...
                var waste = 1 - Vector3.Dot(thrustNorm, direction);
            ...
            alglib.minbleiccreatef(x, diffstep, out var state);
            alglib.minbleicsetbc(state, bndl, bndu);
            alglib.minbleicsetcond(state, epsg, epsf, epsx, maxits);
            alglib.minbleicoptimize(state, cost_func, null, null);
            alglib.minbleicresults(state, out var throttles, out var rep);
            ...
            var fullThrottles = new double[fullCount];
...
// dictionary of previously-calculated throttles.
    public class RCSSolverKey
    {
        ...
        public RCSSolverKey(ref Vector3 d, Vector3 rot)
        {
            ...
            d.Normalize();
            var maxabs = Mathf.Max(Mathf.Abs(d.x), Mathf.Max(Mathf.Abs(d.y), Mathf.Abs(d.z)));
        ...
        {
            return (other is RCSSolverKey oth) && hash == oth.hash;
        }
    ...
    public class RCSSolverThread : IDisposable
    {
        ...
        private void CheckVessel(Vessel vessel, VesselState state)
        {
            ...
            // disabled.
            for (var i = 0; i < lastDisabled.Count; i++)
            {
            ...
            var rootPartBody = vessel.rootPart.rb;
            if (rootPartBody != null)
            {
                ...
                var com = WorldToVessel(vessel, comState - rootPos);
            ...
            for (var index = 0; index < vessel.parts.Count; index++)
            {
                ...
                foreach (var pm in p.Modules.OfType<ModuleRCS>())
                {
                    ...
                    {
                        var pos = VesselRelativePos(state.CoM, vessel, p);
                        ...
                        for (var i = 0; i < pm.thrusterTransforms.Count; i++)
                        {
            ...
            while (workEvent.WaitOne() && !stopRunning)
            {
                try
                {
                    ...
                    while (tasks.Count > 0 && !stopRunning)
                    {
                        ...
                        var throttles = solver.run(thrusters, task.direction, task.rotation);
