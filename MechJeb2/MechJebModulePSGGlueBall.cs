                ...
                if (!hasCoast)
                {
                        ...
                        (kspStage == _ascentSettings.CoastStage - 1 && !_ascentSettings.CoastBeforeFlag))
                    {
                        ...
                        if (_ascentSettings.FixedCoast)
                            ascentBuilder.AddFixedCoast(fuelStats.StartMass * 1000, ct, _ascentSettings.CoastStage, mjPhase, unguidedCoast);
                        else
                            ascentBuilder.AddOptimizedCoast(fuelStats.StartMass * 1000, mint, maxt, _ascentSettings.CoastStage, mjPhase, unguidedCoast);
                    }
                }
